...
const {
  ...
  getCashOutCommision,
} = require("./cashOutComputations.js");
...
test("Computing Commision : compuTeCommision()", () => {
  ...
  expect(compuTeCommision(mockData[1])).toBe(0.9);
  expect(compuTeCommision(mockData[2])).toBe(87.0);
  expect(compuTeCommision(mockData[3])).toBe(2.7);
});
...
test("Computing for cash_out type () commision : getCashOutCommision()", () => {
  ...
  expect(getCashOutCommision(mockData[0])).toBe(0.9);
  expect(cashOutJudirical).toHaveBeenCalledTimes(0);

  const cashOutNatural = jest.fn();
  expect(getCashOutCommision(mockData[1])).toBe(0.3);
...
test("computing for commission fee for natural type cash out with same and new week", () => {
  ...
  const mockAnswer = [0.3, 2.7, 0.3, 0.3, 0.9];

  mockData.forEach((mock, index) => {
    expect(getCommisionForNatural(mock.operation.amount, mock.user_id)).toBe(
      mockAnswer[index]
    );
  });
});
...
test("rounding off to the smallest currency : convertToSmallestCurrency", () => {
  const mockData = [
    ...
    },
    {
      ...
      user_type: "natural",
      type: "cash_in",
      operation: { amount: 423.0, currency: "EUR" },
    },
  ];

  expect(
    convertToSmallestCurrency(
      compuTeCommision(mockData[0]),
      mockData[0].operation.currency
    )
  ).toBe(0.06);
  ...
  expect(
    convertToSmallestCurrency(
      compuTeCommision(mockData[1]),
      mockData[1].operation.currency
    )
  ).toBe(0.13);
});
